...
class buttons:  # класс для создания клавиатур различных категорий товаров
    ...
    async def razdely_buttons(self):
        keys = {}
        ...
        for i in self.list_one:
            ...
            if self.list_one.index(i) > 0 and self.list_one.index(i) % 2 != 0:
                ...
                kb1.add(keys[f'but{self.list_one.index(i)}'])
        await self.bot.send_photo(self.message.chat.id, photo=self.image)
        await self.bot.send_message(self.message.chat.id, text=f'Пожалуйста выберите {self.kategoriya}', reply_markup=kb1)

    async def marks_buttons(self):  # функция создающая клавиатуру
        keys = {}
        kb1 = types.InlineKeyboardMarkup()
        for i in self.list_one:
            ...
            if self.list_one.index(i) > 0 and self.list_one.index(i) % 2 != 0:
                if len(i[0]) <= 16 and len(self.list_one[self.list_one.index(i) - 1][0]) <= 16:
                    ...
                    try:
                        ...
                        await self.bot.send_message(admin_id, 'Ошибка!!!Проверьте таблицу на дублирование')
            elif self.list_one.index(i) == (len(self.list_one) - 1):
                kb1.add(keys[f'but{self.list_one.index(i)}'])
        await self.bot.send_photo(self.message.chat.id, photo=self.image)
        await self.bot.send_message(self.message.chat.id, text=f'Пожалуйста выберите {self.kategoriya}', reply_markup=kb1)

    async def zayavka_buttons(self, back_value='Вернуться в начало'):
        ...
        kb4.add(but1, but3)
        await self.bot.send_message(self.message.chat.id, f'Хотите оформить заявку/купить онлайн выбранный товар?\n '
                                                    f'/help - Подробности покупки', reply_markup=kb4)

    async def basket_buttons(self):
        ...
        await self.bot.send_message(self.message.chat.id, f'Хотите оформить заказ/купить онлайн выбранный товар?\n '
                                                    f'/help - Подробности покупки', reply_markup=kb7)

    async def basket_buttons_redact(self):
        keys = {}
        ...
        try:
            ...
            for i in cell_id:
                if worksheet2.cell(i.row, 16).value == 'FALSE':
                    keys[f'but{cell_id.index(i)}'] = types.InlineKeyboardButton(text=str(int(cell_id.index(i)) + 1),
            ...
            await self.bot.edit_message_text('Выберите номер позиции в которой нужно изменить количество',
                                             self.message.chat.id, self.message.message_id)
            await self.bot.edit_message_reply_markup(self.message.chat.id, self.message.message_id, reply_markup=kb4)
        except Exception:
            await self.bot.send_message(self.message.chat.id, 'Ошибка подключения. Повторите запрос через 1 минуту.')


async def tovar_in_basket(bot, message):
    global ostatok
    ...
    for row in ws.iter_rows(min_row=1, max_row=ws.max_row, min_col=1, max_col=10, values_only=True):
        if row[0] == message.chat.id:
            ...
            await poisk_tovar_in_base(bot, message, row[2], row[1], quantity, size=row[3],
...
async def zayavka_done(bot, message, number):
    ...
    try:
        ...
        await bot.send_message(admin_id, 'Заявка внесена в базу ✅\n'
                                   'смотреть базу: https://docs.google.com/spreadsheets/d/'
                                   '14P5j3t4Z9kmy4o87WEbLqeTwsKi7YZAx7RiQPlY2c1w/edit?usp=sharing')

    except Exception:
        await bot.send_message(message.chat.id, 'Ошибка подключения. Повторите запрос через 1 минуту.')
        await buttons(bot, message).menu_buttons()
...
class poisk_tovar_in_base:
    ...
    async def poisk_ostatok(self, back_value='Вернуться в начало'):
        global ostatok
        try:
            await self.bot.send_message(self.message.chat.id, 'Проверяем наличие..')
            if self.image is not None and self.image != 0:
                await self.bot.send_photo(self.message.chat.id, self.image, f'{self.tovar_name}\nРазмер: {self.size}\n'
                                                                      f'Артикул: {self.article}')
                await self.bot.send_message(self.message.chat.id, f'В наличии: {self.vnalichii}\n'
                ...
                if self.vnalichii == 0:
                    ...
                    kb4.add(but1)
                    await self.bot.send_message(self.message.chat.id, f'Увы товар закончился\n'
                                                                f'/help - справка по боту\n', reply_markup=kb4)
                else:
                    await buttons(self.bot, self.message).zayavka_buttons(back_value=back_value)
                    ostatok = self.vnalichii
            else:
                await self.bot.send_message(self.message.chat.id, f'Фото товара временно отсутствует\n{self.tovar_name}\n'
            ...
            await self.bot.send_message(self.message.chat.id, f'Ошибка, товар отсутствует')

    async def zayavka_v_baze(self, itogo, tovar_kategory):  # функция перевода из базы потенциальных клиентов в базу старых клиентов
        ...
        try:
            ...
            await self.bot.send_message(admin_id, 'Ошибка добавления  товара в базу')

    async def poisk_number(self):
    ...
    async def basket_search(self):
        name = []
        sum_price = 0
        sum_quantity = 0
        await self.bot.send_message(self.message.chat.id, "Собираем данные..")
        ...
        name_ = ' '.join(name)
        if len(name) != 0:
            await self.bot.send_message(self.message.chat.id, f'На данный момент Ваши товары в корзине:\n'
                                                        f'{name_}\n'
                                                        f'Итого в корзине товаров: \n'
                                                        f'в количестве {sum_quantity} шт.\n'
                                                        f'на сумму {sum_price}₽')
            await buttons(self.bot, self.message).basket_buttons()
        else:
            await self.bot.send_message(self.message.chat.id, f'Товары отсутствуют..Проверьте историю заказов.')

    async def basket_delete(self, row):
        row = int(row)
        try:
            ...
            for a in range(1, self.ws2.max_row + 1):
                if str(self.ws2.cell(a, 1).value) == str(article):
                    ...
                    await self.bot.send_message(self.message.chat.id, 'Товар успешно удален из корзины')
                    self.worksheet2.batch_clear([f"A{row}:O{row}"])
                    self.wb.save('CCM.xlsx')
                    break
        except AttributeError:
            await self.bot.send_message(self.message.chat.id, 'Товар уже был удален ранее. Перейдите в корзину снова, чтобы '
                                                        'обновить данные')

    async def basket_delete_all(self):
        try:
            cell_id = (self.worksheet2.findall(str(self.message.chat.id), in_column=1))[::-1]
            for i in cell_id:
                if self.worksheet2.cell(i.row, 16).value == 'FALSE':
                    ...
                    for a in range(1, self.ws2.max_row + 1):
                        if str(self.ws2.cell(a, 1).value) == str(article):
                            ...
                            self.wb.save('CCM.xlsx')
                            break
            await self.bot.send_message(self.message.chat.id, 'Товары успешно удалены')
        except AttributeError:
            await self.bot.send_message(self.message.chat.id, 'Товар уже был удален ранее. Перейдите в корзину снова, чтобы '
                                                        'обновить данные')

    async def zakazy_search(self):
        try:
            ...
            if len(cell_id.keys()) != 0:
                ...
                block = ' '.join(block)
                await self.bot.send_message(self.message.chat.id, f' Ваша история заказов:\n'
                                                            f'{block}')
            else:
                await self.bot.send_message(self.message.chat.id, f'Заказы отсутствуют')
        except Exception:
            await self.bot.send_message(admin_id, f'Сервисное сообщение: Ошибка в оформлении гугл таблицы. У каждого нового '
